...
void Game::initBoard()
{
    ...
    if (playerHuman != NULL) delete playerHuman;
    playerHuman = new Player(this, 0);

    if (playerCompList == NULL)
        playerCompList = new LinkedList<AICore*>();
    while (playerCompList->getLength() > 0)
        delete playerCompList->poll();
    playerCompList->addFirst(new AIBasic(this, 1));
}
...
LinkedList<AICore*>* Game::getAIPlayers()
{
